...
const incrementDebounceCount = (req: IncomingMessage, res: ServerResponse) => {
  debounceCount++;
  res.writeHead(200, { 'Content-Type': 'application/json' });
  res.end(JSON.stringify({ message: 'Debounce count incremented', count: debounceCount }));
};

const incrementThrottleCount = (req: IncomingMessage, res: ServerResponse) => {
  throttleCount++;
  res.writeHead(200, { 'Content-Type': 'application/json' });
  res.end(JSON.stringify({ message: 'Throttle count incremented', count: throttleCount }));
};

export const debouncedIncrement = debounce(incrementDebounceCount, 2000);
export const throttledIncrement = throttle(incrementThrottleCount, 2000);
